...
import { ParametrosOrganizador } from "@/components/parametros-organizador";
...
export function AgendaOrganizador() {
  ...
  const agenda = useMemo(() => {
    ...
    return mergeAgenda(reservas, params).map((turno) => ({
      ...turno,
  ...
  async function onSubmit(event: React.FormEvent) {
    ...
    setError(null);
    try {
      const [rows, loaded] = await Promise.all([listOrganizerReservas(pin), getEventParams()]);
      setReservas(rows);
      setParams(loaded);
    ...
    return (
      <div className="mx-auto flex w-full max-w-md flex-col gap-4 px-4 py-10 sm:px-6">
        ...
        <form onSubmit={onSubmit} className="rounded-xl border-2 border-primary bg-[color:var(--paper)] p-5">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Ingrese el PIN para ver teléfonos, exportar la hoja y editar los parámetros.</p>
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 px-4 py-6 sm:px-6">
      <div className="flex flex-col gap-3 rounded-xl border-2 border-primary bg-[color:var(--paper)] p-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button type="button" variant={tab === "parametros" ? "default" : "outline"} onClick={() => setTab("parametros")}>Parámetros</Button>
          <Button type="button" onClick={exportCsv}>Exportar CSV</Button>
          <Button type="button" variant="outline" onClick={() => { setReservas(null); setPin(""); setTab("reservas"); sessionStorage.removeItem(ORGANIZER_SESSION_KEY); }}>Salir</Button>
        </div>
      </div>

      {tab === "parametros" ? (
        <ParametrosOrganizador
          pin={pin}
          params={params}
          reservas={reservas}
          onSaved={setParams}
